...
Sequel.migration do
  up do
    create_table(:account_password_hashes) do
      foreign_key :id, Sequel[:"#{DB.opts[:database]}"][:accounts], primary_key: true, type: :Bignum
      String :password_hash, null: false
    end
    Rodauth.create_database_authentication_functions(self, table_name: Sequel[:"#{DB.opts[:database]}_password"][:account_password_hashes])    
    case database_type
    when :postgres
    ...
    create_table(:account_previous_password_hashes) do
      ...
      foreign_key :account_id, Sequel[:"#{DB.opts[:database]}"][:accounts], type: :Bignum
      String :password_hash, null: false
    end
    Rodauth.create_database_previous_password_check_functions(self, table_name: Sequel[:"#{DB.opts[:database]}_password"][:account_previous_password_hashes])
